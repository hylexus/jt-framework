package io.github.hylexus.jt808.samples.common.web.reactive;
...
@Slf4j
public class SamplesResponseWrapperResultHandler extends ResponseBodyResultHandler {
    private static final MethodParameter ACTUAL_RETURN_TYPE;

    static {
        try {
            ACTUAL_RETURN_TYPE = new MethodParameter(SamplesResponseWrapperResultHandler.class.getDeclaredMethod("actualReturnType"), -1);
    ...
    public SamplesResponseWrapperResultHandler(
    ...
    public Mono<Void> handleResult(@NonNull ServerWebExchange exchange, @NonNull HandlerResult result) {
        ...
        if (returnValue == null) {
            return this.writeData(Resp.empty(), ACTUAL_RETURN_TYPE, exchange);
        }
