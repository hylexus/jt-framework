...
public class CommonHandler {
    ...
    public ServerCommonReplyMsgBody processAuthMsgV2011(AuthRequestMsgV2011 msgBody, RequestMsgHeader header, Jt808Session abstractSession, Session session) {
    ...
    public ServerCommonReplyMsgBody processAuthMsgV2019(AuthRequestMsgV2019 msgBody, RequestMsgHeader header, Jt808Session abstractSession, Session session) {
        log.info("处理鉴权消息 terminalId = {}, authCode = {}", header.getTerminalId(), msgBody.getAuthCode());
        if (header.getTerminalId().equals(System.getProperty("debug-terminal-id"))) {
            throw new UnsupportedOperationException("terminal [" + header.getTerminalId() + "] was locked.");
        }
