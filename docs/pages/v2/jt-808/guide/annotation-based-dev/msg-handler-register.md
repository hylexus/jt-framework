...
-
    1.
    本小节的示例代码可以在 [jt-framework-samples-maven/jt-808-server-sample-annotation](https://github.com/hylexus/jt-framework-samples/tree/master/jt-808-server-sample-annotation)
    下找到对应代码。
-
...
- 从逻辑上来理解，被其标记的方法相当于一个实现了 `MsgHandler` 接口的处理器类。
...
底层由 `HandlerMethodArgumentResolver` 来完成注入。 所有支持的参数解析器都委托给了 `DelegateHandlerMethodArgumentResolvers`。
