...
/**
 * 分析由旧版 1078 消息处理线程池 Bean 导致的 Provider 注入失败。
 *
 * @author hylexus
 * @author Codex
...
public class Jt1078EventExecutorGroupProviderFailureAnalyzer extends AbstractFailureAnalyzer<NoSuchBeanDefinitionException> {
    private static final String ISSUE_URL = "https://github.com/hylexus/jt-framework/issues/101";

    private final ConfigurableListableBeanFactory beanFactory;

    public Jt1078EventExecutorGroupProviderFailureAnalyzer(BeanFactory beanFactory) {
        ...
        while (current != null) {
            if (current instanceof UnsatisfiedDependencyException) {
                ...
                if (injectionPoint != null && JtEventExecutorGroupProvider.class.equals(injectionPoint.getDeclaredType())) {
                    ...
                    if (qualifier != null && BEAN_NAME_1078_MSG_PROCESSOR_EVENT_EXECUTOR_GROUP.equals(qualifier.value())) {
