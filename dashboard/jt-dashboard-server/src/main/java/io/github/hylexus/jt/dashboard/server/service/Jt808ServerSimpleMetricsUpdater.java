 ...
 */
@Slf4j
public class Jt808ServerSimpleMetricsUpdater {
    private final Jt808InstanceRegistry instanceRegistry;
    private final DashboardWebClient dashboardWebClient;

    public Jt808ServerSimpleMetricsUpdater(Jt808InstanceRegistry instanceRegistry, DashboardWebClient.Builder builder) {
        this.instanceRegistry = instanceRegistry;
        this.dashboardWebClient = builder.build();
    }

    public Mono<Void> updateStatus(String id) {
        return this.instanceRegistry.getInstance(id).map(this::doUpdateStatus).orElseGet(Mono::empty);
    }

    protected Mono<Void> doUpdateStatus(JtInstance instance) {
        log.debug("Update status for {}", instance);
        return this.dashboardWebClient.mutateForInstance(instance)
                .get()
                .uri("/actuator/jt808SimpleMetrics")
